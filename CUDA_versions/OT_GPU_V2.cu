...
//warpings worden volledig gevuld in deze versie
...
void save_coordinates_to_file(const std::vector<std::pair<int, int>>& coordinates) {
    std::ofstream outFile("./output/coordinates_cuda_V2.txt"); //./output/coordinates_cuda_nolock.txt
...
int main() {
    ...
    for (int frameNr = 0; frameNr < 150; ++frameNr) {
        ...
        find_object_colour<<<grid_size, row_block_size>>>(d_pixels, d_row_counts, d_col_counts); //this one has two block with each 1024 threads => we have 64 warps and the penultimate one
                                                                                                 //uses 16 out of it's 32 threads (the other 16 idle) and the last one uses none of it's 32
                                                                                                 //threads (all of them idle)
        cudaEventRecord(stop);
        //find_object_colour<<<(H + W), 1>>>(d_pixels, d_row_counts, d_col_counts); //this one uses 2000 blocks with each one thread (the other 31 threads of each block are idling)
                                                                                  //=> the GPU resources have been overprovisioned                                                          
